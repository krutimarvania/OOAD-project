...
def searchWord(): 
    if request.method == 'POST':
        ...
        # print(">>>>>>",posted_data)
        words = posted_data['searchInput']  
        print(">>>> wors : ", words)      # Fetch the data from request payload
        allColumnsDict = getData()                # All columns dict retrival from getData()  
        print(">> all column : ", allColumnsDict)
        ...
        for title in allColumnsDict:
            ...
            if words.lower() in title.lower() or words.lower() in allColumnsDict[title][0].lower() or words.lower() in allColumnsDict[title][1].lower():
                ...
                # title1 = title.tolower()
               
                urlDescDict['title'] = title    
                urlDescDict['url']= allColumnsDict[title][1]
                urlDescDict['description'] =  allColumnsDict[title][0]
